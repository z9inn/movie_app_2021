...
class App extends React.Component{
  ...
  getMovies = async () => { // async는 자바스크립트에게 getMovies() 함수는 시간이 필요하고
                            // async는 JS에게 getMovies() 함수가 비동기라고 말해주는 것. JS에게 getMovies() 함수는 비동기라서 기다려야 해라고 말한거라고 생각하면 됨
    const movies = await axios.get('https://yts-proxy.now.sh/list_movies.json'); // await.get()의 실행을 기다려달라고 말해주는 것
